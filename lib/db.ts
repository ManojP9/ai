...
// ── course_lessons: AI Engineering From Scratch tracker ──────────────────────
// Mirrors the version1 tree (phase rows = parent_id NULL, lessons = children)
// but tracks the external rohitg00/ai-engineering-from-scratch curriculum.
// Catalog is generated by scripts/sync-curriculum.mjs.
...
export type CourseNode = CourseRow & { children: CourseNode[] };

export async function initCourse() {
  await sql`
    CREATE TABLE IF NOT EXISTS course_lessons (
      id          SERIAL PRIMARY KEY,
      parent_id   INTEGER REFERENCES course_lessons(id) ON DELETE CASCADE,
      title       TEXT NOT NULL,
      phase       INTEGER,
      order_index INTEGER DEFAULT 0,
      status      TEXT NOT NULL DEFAULT 'pending',
...
export async function updateCourseLesson(id: number, status: TaskStatus) {
  await initCourse();
  await sql`UPDATE course_lessons SET status = ${status} WHERE id = ${id}`;
}

export async function getCourse(): Promise<CourseNode[]> {
  await initCourse();
  const { rows } = await sql<CourseRow>`
    SELECT * FROM course_lessons ORDER BY phase NULLS LAST, parent_id NULLS FIRST, order_index, id
  `;
...
// Single source of truth for stats + next-lesson, derived from the tree.
export function deriveStats(tree: CourseNode[]): { stats: CourseStats; next: NextLesson } {
  const lessons = tree.flatMap((p) => p.children);
  const done = lessons.filter((l) => l.status === "done");
  const total = lessons.length;
  const hoursTotal = lessons.reduce((s, l) => s + (l.hours || 0), 0);
  const hoursDone = done.reduce((s, l) => s + (l.hours || 0), 0);

  const perPhase = tree.map((p) => {
    const d = p.children.filter((l) => l.status === "done").length;
    return {
      phase: p.phase ?? 0,
      title: p.title,
      total: p.children.length,
      done: d,
      pct: p.children.length ? Math.round((d / p.children.length) * 100) : 0,
    };
  });

  // Next lesson = first non-done lesson, phases already ordered by getCourse.
  let next: NextLesson = null;
  for (const p of tree) {
    const hit = p.children.find((l) => l.status !== "done");
    if (hit) {
      next = {
        id: hit.id,
        title: hit.title,
        phase: hit.phase,
        phaseTitle: p.title,
        docsUrl: hit.docs_url,
        codeUrl: hit.code_url,
      };
      break;
    }
  }
  ...
  return {
    stats: {
      ...
      hoursRemaining: Math.round(hoursTotal - hoursDone),
      perPhase,
    },
    next,
  };
...
// Builds a "($1,$2,...),($n,...)" VALUES clause for `cols`-wide rows.
function valuesClause(rowCount: number, cols: number): string {
  return Array.from({ length: rowCount }, (_, r) =>
    `(${Array.from({ length: cols }, (_, c) => `$${r * cols + c + 1}`).join(",")})`
  ).join(",");
}

export async function seedCourse(): Promise<{ seeded: boolean; phases: number; lessons: number }> {
  await initCourse();
  const { rows: existing } = await sql`SELECT COUNT(*)::int AS count FROM course_lessons`;
  if ((existing[0] as { count: number }).count > 0) {
    return { seeded: false, phases: 0, lessons: 0 };
  }

  // 1) Insert all phase rows in one statement; map phase number -> generated id.
  const phaseParams: (string | number)[] = [];
  curriculum.phases.forEach((phase, i) => {
    phaseParams.push(`Phase ${phase.num} — ${phase.title}`, parseInt(phase.num, 10), i);
  });
  const { rows: phaseRows } = await sql.query<{ id: number; phase: number }>(
    `INSERT INTO course_lessons (title, phase, order_index)
     VALUES ${valuesClause(curriculum.phases.length, 3)}
     RETURNING id, phase`,
    phaseParams
  );
  const phaseIdByNum = new Map(phaseRows.map((r) => [r.phase, r.id]));

  // 2) Insert all lessons in one statement, parented to the phase ids above.
  const lessonParams: (string | number | null)[] = [];
  let lessonCount = 0;
  for (const phase of curriculum.phases) {
    const phaseNum = parseInt(phase.num, 10);
    const parentId = phaseIdByNum.get(phaseNum)!;
    phase.lessons.forEach((lesson, j) => {
      lessonParams.push(
        parentId, `${lesson.num} — ${lesson.title}`, phaseNum, j,
        lesson.slug, lesson.docsUrl, lesson.codeUrl, lesson.hours
      );
      lessonCount++;
    });
  }
  await sql.query(
    `INSERT INTO course_lessons (parent_id, title, phase, order_index, slug, docs_url, code_url, hours)
     VALUES ${valuesClause(lessonCount, 8)}`,
    lessonParams
  );

  return { seeded: true, phases: phaseRows.length, lessons: lessonCount };
}
